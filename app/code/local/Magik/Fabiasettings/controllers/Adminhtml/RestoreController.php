...
class Magik_Fabiasettings_Adminhtml_RestoreController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('magik/fabia/restore')
            ->_addBreadcrumb(Mage::helper('fabiasettings')->__('Restore Defaults'), Mage::helper('fabiasettings')->__('Restore Defaults'));
    ...
    public function indexAction()
    {
        ...
        $this->_title($this->__('Magik'))
            ->_title($this->__('Fabia'))
            ->_title($this->__('Restore Defaults'));

        $this->_addContent($this->getLayout()->createBlock('fabiasettings/adminhtml_restore_edit'));
    ...
    public function restoreAction()
    {
	    ...
	    try { 
            ...
            $defaults->loadFile(Mage::getBaseDir().'/app/code/local/Magik/Fabiasettings/etc/config.xml');
            $this->_restoreSettings($defaults->getNode('default/fabiasettings')->children(), 'fabiasettings');

		    if ($setup_cms) {
                Mage::getModel('fabiasettings/settings')->setupCms();
            }

            Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('fabiasettings')->__('Fabia Theme Settings has been restored. Please clear cache (System > Cache management) if you do not see changes in storefront'));
        }
        catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('fabiasettings')->__('An error occurred while restoring theme settings.'));
        }
