...
class Magestore_Affiliateplusprogram_Model_Observer extends Varien_Object {
    ...
    public function addressCollectTotal($observer) {
        ...
        $baseDiscountExclTax = $discountObj->getBaseDiscount();
        ...
        foreach ($affiliateInfo as $info)
            if ($account = $info['account']) {
                        ...
                        if ($discountType == 'cart_fixed') {
                            ...
                            foreach ($address->getAllItems() as $_item) {
                                ...
                                if ($_item->getHasChildren() && $_item->isChildrenCalculated()) {
                                    foreach ($_item->getChildren() as $child) {
										...
										/* Changed By Adam: 26/08/2015: Fix issue calculate grandtotal after using affiliate discount when calculate discount include tax and calculate tax after discount  */
										$baseItemsPriceExclTax += $_item->getQty() * ($child->getQty() * $child->getBasePrice() - $child->getBaseDiscountAmount());
                                    }
									...
									/* Changed By Adam: 26/08/2015: Fix issue calculate grandtotal after using affiliate discount when calculate discount include tax and calculate tax after discount  */
									$baseItemsPriceExclTax += $_item->getQty() * $_item->getBasePrice() - $_item->getBaseDiscountAmount();
                                }
                            }
                            if ($baseItemsPrice) {
                            ...
                            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                                ...
                                if ($applyTaxAfterDiscount) {
									...
									// print_r("taxableAmount".$taxableAmount); echo "<br>";
									// print_r("itemTaxamou: ".$taxableAmount - $item->getTaxableAmount()); echo "<br>";
									// print_r($item->getTaxableAmount()); echo "<br>";
									// print_r($item->getAffiliateplusHiddenTaxAmount()); echo "<br>";
									// echo "<br><br>";
				...
				/* Changed By Adam: 26/08/2015: Fix issue calculate grandtotal after using affiliate discount when calculate discount include tax and calculate tax after discount  */
                $discountObj->setBaseDiscountExclTax($baseDiscountExclTax);
...
//                $commissionObj->setAffiliateplusCommissionItem($affiliateplusCommissionItem);
    ...
    public function round($price) {
        return Mage::getSingleton('tax/calculation')->roundUp($price);
    }
