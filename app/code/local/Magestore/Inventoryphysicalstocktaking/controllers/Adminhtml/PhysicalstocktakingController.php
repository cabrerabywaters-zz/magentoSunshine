 ...
 */
class Magestore_Inventoryphysicalstocktaking_Adminhtml_PhysicalstocktakingController extends Magestore_Inventoryplus_Controller_Action {
                ...
                if ($this->getRequest()->getParam('confirm') || $this->getRequest()->getParam('confirmadjust')) {
                    ...
                    if ($this->getRequest()->getParam('confirmadjust')) {
                        ...
                        if ($adjustModel->getStatus() == Magestore_Inventoryplus_Model_Adjuststock::STATUS_PROCESSING) {
                            return $this->_redirect('inventoryplusadmin/adminhtml_adjuststock/run', array('id' => $adjustModel->getId(), 'resum' => 1));
                        }
                        $url = Mage::helper('adminhtml')->getUrl('inventoryplusadmin/adminhtml_adjuststock/edit', array('id' => $adjustModel->getId()));
                        Mage::getSingleton('adminhtml/session')->addSuccess(
