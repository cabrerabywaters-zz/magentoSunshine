...
class Magestore_Webpos_Helper_Product extends Mage_Core_Helper_Abstract {
	...
	protected function getProductsByWarehouseSession($warehouseId){ //Magnus
		...
		$sql = "SELECT `main_table`.`product_id` FROM ".$coreResource->getTableName('erp_inventory_warehouse_product')." AS `main_table` WHERE (warehouse_id = '".$warehouseId ."') AND (available_qty > 0)";
