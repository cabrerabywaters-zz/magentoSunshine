    ...
    protected function _prepareCollection() {
		...
		if($timeFrom && $timeTo){
                    while($timeFrom < $timeTo){
                        ...
                        foreach($this->userIds as $userId => $displayName){
                                ...
                                $webposOrderCollection = $this->getSalesCollection($timeFrom,$endTime,array('user_id' => $userId))->getFirstItem();
		...
		$this->setCollection($collection);
                ...
                foreach($this->userIds as $userId => $username){
                    $totalSalesByLocation = $this->getSalesTotal($startTime,$endTime,array('user_id' => $userId))
                                            ->getFirstItem()->getTotals();
                    $sessionObject->setData($username,$totalSalesByLocation);
                }
