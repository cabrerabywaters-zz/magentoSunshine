...
class Magestore_Webpos_Block_Adminhtml_Report_Location extends Magestore_Webpos_Block_Adminhtml_Report_Abstract {
    ...
    protected function _prepareCollection() {	
                ...
                if($timeFrom && $timeTo){    
                    while($timeFrom < $timeTo){
                        ...
                        foreach($this->locationIds as $locationId => $displayName){
                                ...
                                $webposOrderCollection = $this->getSalesCollection($timeFrom,$endTime,array('user_location_id' => $locationId))->getFirstItem();
                                ...
                                if($isSave){
                                    ...
                                    $itemDataObject->setData('totals_sales',$webposOrderCollection->getTotals()?$webposOrderCollection->getTotals():'0.00');
		...
		$order = $this->getSalesTotal($startTime,$endTime)->getFirstItem();
                if(isset($order)) {
                    $salesData = $order->getTotals();
                    $lastItemDataObject->setData('totals_sales',$salesData);	
                }
		...
		$this->setCollection($collection);
                ...
                foreach($this->locationIds as $locationId => $locationName){
                    $totalSalesByLocation = $this->getSalesTotal($startTime,$endTime,array('user_location_id' => $locationId))
                                            ->getFirstItem()->getTotals();
                    $sessionObject->setData($locationName,$totalSalesByLocation);
                }
