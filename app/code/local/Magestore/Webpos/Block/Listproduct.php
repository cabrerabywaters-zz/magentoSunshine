...
class Magestore_Webpos_Block_Listproduct extends Mage_Core_Block_Template {
	...
	public function __construct() {
		...
		if(isset($pageNumber)) $this->_currentPage = $pageNumber;	
		else $this->_coreSession->setData('pos_cpage',1);
...
//            $query = "SELECT * FROM $table where store_id=$storeId AND data_index like '%$keyword%'";
		...
		if(!isset($storeId)) $storeId = $this->_storeId;
		if(!isset($pageNumber)) $pageNumber = $this->_currentPage;
		if(!isset($numberProductPerpage)) $numberProductPerpage = $this->_numberProductPerpage;
...
//                    ->setCurPage($pageNumber);
                ...
                if ($useLocalSearch==true) {
                    ...
                    else $this->_coreSession->setData('numberProduct',$this->_productCollection->getSize());
                }
...
//                        $this->_productCollection->addAttributeToFilter('entity_id', array('in'=>$productIds));
		...
		if(isset($categoryId) && $categoryId != 0){
			...
			$this->_productCollection->getSelect()->where('t1.category_id IN('.$categories.')');
		}
		Mage::getSingleton('catalog/product_status')->addSaleableFilterToCollection($this->_productCollection);
		//Mage::getSingleton('catalog/product_visibility')->addVisibleInSiteFilterToCollection($this->_productCollection);
        if (!$showOutofstock) Mage::getSingleton('cataloginventory/stock')->addInStockFilterToCollection($this->_productCollection);
		Mage::dispatchEvent('webpos_block_listproduct_event', array('pos_get_product_colection' => $this->_productCollection));
		Mage::getSingleton('core/session')->setData('numberProduct',$this->_productCollection->getSize());
