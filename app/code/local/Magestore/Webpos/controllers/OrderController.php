...
class Magestore_Webpos_OrderController extends Magestore_Webpos_Controller_Action {
    ...
    public function emptyCartAction() {
        ...
        try {
			...
			if ($customerSession->isLoggedIn()) $customerSession->logout();
			if (version_compare(Mage::getVersion(), '1.7.0.0', '>')) $customerSession->renewSession();
        } catch (Exception $e) {
    ...
    public function saveOrderAction() {
		...
		if(!$allowGuestCheckout && !$customerSession->isLoggedIn()) $customerSession->setCustomerAsLoggedIn($customer);
        if (!$quote->getCustomer()) {
            
			//$helper->deleteCustomerQuote($customer->getId());
        ...
        try {
			...
			$result = Mage::helper('webpos')->savePaymentMethod($payment);
        ...
        if (!$error) {
            ...
            if ($redirectUrl) {
                ...
                $printLink = Mage::getUrl('pdfinvoiceplus/order/print', array('order_id' => $lastOrderId, '_secure' => true));
				...
				*/
                if ($order->getId() != '') {
                    ...
                    $result['paidFullAmount'] = $paidFullAmount;
                    if ($isVirtual)
    ...
    public function saveOrderData($order) {
        ...
        $posorderModel->setData('user_location_id', $userLocationId);
    ...
    public function refundOrderAction() {
       ...
       if($collectionCreditmemo->getSize()){
        foreach($collectionCreditmemo as $co){
         $shipRefuned += $co->getShippingAmount();
        }
       }
       ...
       /**/
        $other = array(
            ...
            "shipping_amount" => $oldShip?($oldShip - $shipRefuned):'0',
...
//                $ord = $order->setEmailSent(false)->setCustomerEmail($orderCustomerEmail)->queueOrderUpdateEmail(true, '', true);
	...
	public function savePendingOrder($orderData){
		if(empty($orderData)) return array('error' => $this->__('Order data is empty!'));
		$error = false;
		...
		if(count($cartData) > 0){
			...
			/*
			$saveResult = $posorder->createOrder($products);
			*/
		...
		if(isset($orderResult) && !empty($orderResult)){
			...
			if($totalDue <= 0){
				...
				$status = ($create_shipment == 1)?'complete':'processing';
			}else{
				$totalRefunded = 0;
				$this->createShipmentAndInvoice($orderId,$order,$create_shipment,$create_invoice);
				if($create_shipment == 1 && $create_invoice == 1){
					$status = 'complete';
					$totalPaid = $grandTotal;
				}elseif($create_invoice == 1){
					$status = 'processing';
					$totalPaid = $grandTotal;
				}elseif($create_shipment == 1){
					$status = 'processing';
				}
			}
            ...
            $user = $posSession->getUser();
            if ($posUserId != '') {
				$order->setWebposAdminId($posUserId)
				->setWebposAdminName($posUserDisplayName);
            }
            $order->setTotalPaid($totalPaid)
				->setBaseTotalPaid($totalPaid)
				->setTotalRefunded($totalRefunded)
