...
class Magestore_Webpos_IndexController extends Magestore_Webpos_Controller_Action {
    ...
    public function indexAction() {
		...
		if($wid = $this->getRequest()->getParam('warehouse_id'))
			Mage::getSingleton('core/session')->setCurrentWarehouseId($wid );	//Magnus
        /* Daniel - updated - fix error - need to research customer after refesh Web POS page 
          $checkoutSuccess = Mage::getSingleton('checkout/session')->getData('checkout_success');
          if (Mage::getSingleton('customer/session')->isLoggedIn() && $checkoutSuccess == true) {
          Mage::getSingleton('customer/session')->logout();
          Mage::getSingleton('checkout/session')->setData('checkout_success', false);
          }
          end */

        /* Daniel - updated - set default customer */
...
//        }
        ...
        /*
          ...
          if(isset($cookiesData['webpos_admin_key'])){
          $keyo  = $cookiesData['webpos_admin_key'];
          Mage::getModel('webpos/file')->writeFile($data_wp,$file);
          }

          $cookie = Mage::getSingleton('core/cookie');
          $coreCookiesData = $cookie->get();

          if(isset($cookiesData['adminhtml']) && $cookiesData['adminhtml'] != ''){
          $adminhtml  = $cookiesData['adminhtml'];
          }elseif(isset($coreCookiesData['adminhtml']) && $coreCookiesData['adminhtml'] != ''){
          $adminhtml  = $coreCookiesData['adminhtml'];
          }

          if(isset($cookiesData['websiteid'])){
         */
        //Mage::init($cookiesData['websiteid'], 'website');
        /*
          }
          if(isset($cookiesData['storeid'])){
          Mage::app()->setCurrentStore($cookiesData['storeid']);
          }
          $adminkey = Mage::app()->getRequest()->getParam('adminkey');
          if(isset($adminkey))
          $cookiesData['webpos_admin_key'] = $adminkey;
          if((isset($adminkey) && $adminkey != '') || ( isset($coreCookiesData['webpos_admin_key']) && $coreCookiesData['webpos_admin_key'] != '') || ( isset($cookiesData['webpos_admin_key']) && $cookiesData['webpos_admin_key'] != '')) {
          ...
          return $this->_redirect('webposadmin/adminhtml_webpos/index/');
          }
          }
         */
        /* end */
        ...
        if ($customerId) {
            ...
            try {
                if ($customer && $customer->getId()) {
                    try {
                        $oldcustomerId = $posSession->getData('webpos_customerid');
                        if ($oldcustomerId != $customerId) {
                    ...
                    if ($shippingAddressId) {
                        ...
                        if (!empty($shippingAddress)){
                            $this->getOnepage()->saveShipping($shipping_data, '');
							$sameAsBilling = false;
						}
                    }
                    ...
                    if ($billingAddressId) {
                        ...
                        if (!empty($billingAddress)){
                            $this->getOnepage()->saveBilling($billing_data, '');
							if($sameAsBilling) $this->getOnepage()->saveShipping($billing_data, '');
						}
                    }
                    ...
                    if ($resultLogin) {
                        if ($session->getUser()->getStatus() == 2) {
                            ...
                            $storeViewBlock = $this->getLayout()
                                ->createBlock('page/switch', 'store_language')
...
//        $this->renderLayout();
    ...
    public function createCustomerAction() {
        ...
        if (!$result['error']) {
            ...
            //Save billing and shipping address to quote
            $this->getOnepage()->saveBilling($billing_data, '');
            $this->getOnepage()->getQuote()->getBillingAddress()->setEmail($billing_data['email'])->save();
            $this->getOnepage()->saveShipping($billing_data, '');
    ...
    public function saveShippingAddressAction() {
        ...
        if ($shipping_data['customer_id'] && $save_in_address_book) {
