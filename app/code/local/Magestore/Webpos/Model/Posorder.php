...
class Magestore_Webpos_Model_Posorder extends Mage_Core_Model_Abstract
{
    ...
    public function createOrder($products)
    {
		...
		if($this->_billingAddress != ''){
			...
			foreach($billingAddress as $key => $value){
				if(isset($bill[$key]) && $bill[$key] != '') $billingAddress->setData($key,$bill[$key]);
			}
		}
		...
		if($this->_shippingAddress != ''){
			...
			foreach($shippingAddress as $key => $value){
				if(isset($ship[$key]) && $ship[$key] != '') $shippingAddress->setData($key,$ship[$key]);
			}
		}
	...
	public function saveOrderFromQuote(){
		...
		if($this->_billingAddress != '') $this->_quote->getBillingAddress()->addData($this->_billingAddress);
		if($this->_shippingAddress != '') $this->_quote->getShippingAddress()->addData($this->_shippingAddress);
