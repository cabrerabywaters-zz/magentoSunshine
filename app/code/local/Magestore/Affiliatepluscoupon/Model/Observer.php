...
class Magestore_Affiliatepluscoupon_Model_Observer {
	...
	*/
	public function webPOSApplyDiscountAction($observer) {
        ...
        $code = trim($action->getRequest()->getParam('couponCode'));
        if (!$code) {
            return $this;
		}
        ...
        $update->load('webpos_index_applydiscount');
        $layout->generateXml();
	...
	*	Changed By Adam (21/08/2015): using affiliate coupon in webpos
	*/
	public function webPOSremoveCouponAction($observer) {
		$session = Mage::getSingleton('checkout/session');
		$session->setData("coupon_code", null);
