...
class Magestore_Inventoryplus_Model_Warehouse extends Mage_Core_Model_Abstract {
     ...
     * @return \Magestore_Inventoryplus_Model_Warehouse
     */
    public function updateStock($productId, $physicalQty, $availableQty){
        $WHProduct = $this->getProducts(array($productId))->getFirstItem();
        if($WHProduct->getId()){
            $WHProduct->setTotalQty($WHProduct->getTotalQty() + $physicalQty)
                    ->setAvailableQty($WHProduct->getAvailableQty() + $availableQty)
                    ->setUpdatedAt(now());
        } else {
