...
class Magestore_Inventorydashboard_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getStockmovementChartPieData($collection, $requestData) {
        ...
        foreach ($collection as $col) {
            ...
            $series .= '{name:\'' . $alias . '( ' . Mage::helper('inventoryreports')->getMovementTypeText($col->getData('type')) .': '. abs($col->getData('transactionproductqty')).' )\',y:' . abs($col->getData('transactionproductqty')) . '}';
            $i++;
        }
        $data['series'] = $series;
        return $data;
