 ...
 */
   ...
   */
    ...
    /**
     * Event rss_order_new_collection_select
     */
    public function rssOrderNew(Varien_Event_Observer $observer)
    {
        /** @var Mage_Sales_Model_Resource_Order_Collection $collection */
        $collection = $observer->getEvent()->getCollection();

        $role = Mage::getSingleton('aitpermissions/role_front');

        if ($role->isPermissionsEnabled()) {
            $allowedStoreviews = $role->getAllowedStoreviewIds();
            if (count($allowedStoreviews)) {
