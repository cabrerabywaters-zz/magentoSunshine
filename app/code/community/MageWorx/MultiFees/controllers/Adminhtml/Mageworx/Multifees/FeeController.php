...
class MageWorx_MultiFees_Adminhtml_Mageworx_Multifees_FeeController extends  Mage_Adminhtml_Controller_Action
{
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('sales/mageworx_multifees');
    }
    ...
    public function indexAction() {
        ...
        $this->_title($title);
        $this->loadLayout()
            ->_setActiveMenu('sales/mageworx_multifees')
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('mageworx_multifees/fee')->load($id);
        if ($id==0 || $model->getId()) {
    ...
    public function newConditionHtmlAction() {
        ...
        if ($model instanceof Mage_Rule_Model_Condition_Abstract) {
            ...
            $html .= '<script>changeConditionsBasedOnType($(\'fee_type\'));</script>';
        } else {
            $html = '';
        }
    ...
    public function saveAction() {
        ...
        if ($data) {
            $data = Mage::helper('mageworx_multifees')->getFilter($data, false);
            $modelFee = Mage::getSingleton('mageworx_multifees/fee');
            $modelOption = Mage::getSingleton('mageworx_multifees/option');
            $modelStore = Mage::getSingleton('mageworx_multifees/store');
            $modelLngFee = Mage::getSingleton('mageworx_multifees/language_fee');
            $modelLngOption = Mage::getSingleton('mageworx_multifees/language_option');
                    ...
                    if ($options['title']) {
                        foreach ($options['title'] as $key => $value) {
                            ...
                            if (isset($options['image_delete'][$optionId]) && $options['image_delete'][$optionId]==$optionId) {
                                Mage::helper('mageworx_multifees')->removeOptionFile($optionId);
                            }
                            // Upload Img File
    ...
    private function _uploadImage($keyFile, $optionId) {
        if (isset($_FILES[$keyFile]['name']) && $_FILES[$keyFile]['name'] != '') {
            try {
                ...
                $uploader->save(Mage::helper('mageworx_multifees')->getOptionImgPath($optionId), $_FILES[$keyFile]['name']);
            } catch (Exception $e) {
                if ($e->getMessage()) {
                    Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                }
        ...
        if (!is_array($feeIds)) {
            ...
            try {
                foreach ($feeIds as $feeId) {
                    Mage::getModel('mageworx_multifees/fee')->load($feeId)->delete();
                }
    ...
    public function massStatusAction() {
        ...
        if (!is_array($feeIds)) {
            ...
            try {
                foreach ($feeIds as $feeId) {
                    Mage::getSingleton('mageworx_multifees/fee')
                            ->load($feeId)
