	...
	foreach ($groupedData as $group => $groupNodes) {
			...
			foreach ($groupNodes as $node => $persona) {
					...
					else if($persona["Levels"] == 1 && $persona["LiderType"]=="No Lider")
					{
						...
						if($persona["underLider"]==false && $persona["underIndirectLider"]==false && $persona["LiderType"] == "No Lider")
						{
							$groupedData[$group][$node]["head_take_rate"]= $head_take_rate;
						}
...
function stripAccents($str) {
    return strtr(utf8_decode($str), utf8_decode('àáâãäçèéêëìíîïñòóôõöùúûüýÿÀÁÂÃÄÇÈÉÊËÌÍÎÏÑÒÓÔÕÖÙÚÛÜÝ'), 'aaceeeeiiiinooooouuuuyyAAAAACEEEEIIIINOOOOOUUUUY');
}
