...
function showCashIn() {
    if (!$D('#bt_cashin').hasClass('active')) {
		...
		$('bt_cashin').innerHTML = 'Métodos de Pago';
    } else {
...
var addToCartByJs = function(productId, tax_amount, productPrice, imgPath, options, optionsValues, optionsLabels, editing) {
	...
	// currency symbol
	// Changed By Tuan bun (27/08/2015): collect total js
	collectTotalJS();
	collectGrandTotalJS();
	// Changed By Tuan bun (27/08/2015): collect total js
	// end currency symbol
...
var prepareCartItem = function(productId, tax_amount, qty, productPrice, imgPath, options, optionsValues, optionsLabels, editing, oldOptionValues) {
    ...
    for (var j = 0; j < itemChilds.length; j++) {
        ...
        if (itemChilds[j].hasClassName('img-product')) {
            ...
            itemChilds[j].down('.number').innerHTML = parseInt(newqty);
            if (newqty > 1)
                itemChilds[j].down('.number').removeClassName('hide');
            else
                itemChilds[j].down('.number').addClassName('hide');
...
var webpos_save_data = function(save_data_url, type, callback) {
    ...
    var request = new Ajax.Request(save_data_url, {
        ...
        onSuccess: function(transport) {
            if (transport.status == 200) {
				...
				if(response.pos_items && $('webpos_cart')) $('webpos_cart').update(response.pos_items);
				if(response.number_item && $('total_number_item')) $('total_number_item').update(response.number_item);
				// Changed By Tuan bun (27/08/2015): update item when save payment
                if (response.shipping_method && $('shipping_method'))
                    $('shipping_method').update(response.shipping_method);
                if(response.totals && $('pos_totals')) $('pos_totals').update(response.totals);
            ...
            hideColrightAjaxloader();
            hasAnotherRequest = false;
			// Changed By Tuan bun (1/09/2015): save discount 
...
var calculateRemain = function() {
    ...
    if ($('cashin_value')) {
		...
		// Changed By Tuan bun (27/08/2015): recalculate remain
        ...
        if ($('remain_value'))
            $('remain_value').innerHTML = getPriceFormatedHtml(parseFloat(remain));
