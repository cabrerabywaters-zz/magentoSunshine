function isOffline(){
	if(enableOffline == false) return false;
	var status = true;
...
var saveCustomOption = function(productId,optionId,optionValue){
	...
	if($D.jStorage.get('custom_options') != null){
		...
		if(custom_options[productId] == null) custom_options[productId] = {};
...
var getCustomOptionValue = function(productId,optionId){
	...
	if($D.jStorage.get('custom_options') != null){
		custom_options = JSON.parse($D.jStorage.get('custom_options'));
		custom_options[productId][optionId];
		...
		if(typeof custom_options[optionId] === 'array'){
			...
			if($(productId+'_co_'+optionId+'_value')){
				...
				options.each(function(op){
					if(custom_options[optionId].indexOf(op.value) >= 0) op.setAttribute('selected','selected');
				});
			}
		}else
		if($(productId+'_co_'+optionId+'_value'))
...
var getTaxRate = function(countryId,regionId, postcode){
	...
	if($D.jStorage.get('taxCalculations') != null){
		...
		for(var calculationId in taxCalculations){
			...
			if(taxCalculationData.tax_country_id != countryId) thisRate = false;
			if(taxCalculationData.tax_region_id != regionId) thisRate = false;
			if(taxCalculationData.tax_postcode != '*' && taxCalculationData.tax_postcode != postcode) thisRate = false;
			if(taxCalculationData.zip_from != null && taxCalculationData.zip_from < postcode) thisRate = false;
			if(taxCalculationData.zip_to != null && taxCalculationData.zip_to > postcode) thisRate = false;
			if(thisRate == true) return taxCalculationData.rate;
		}
	}
	return 0;
}
...
var saveOutofstockProduct = function(productIds){
	if(productIds == null || productIds == '') return;
	var productsInfo = {};
	if($D.jStorage.get('productsInfo') != null){
		productsInfo = JSON.parse($D.jStorage.get('productsInfo'));
		for(var productId in productsInfo){
...
var saveUpdatedProduct = function(product_updated_data){
	if(product_updated_data == null || product_updated_data == '') return;
	var productsInfo = {};
	if($D.jStorage.get('productsInfo') != null){
		productsInfo = JSON.parse($D.jStorage.get('productsInfo'));
		for(var productId in product_updated_data){
			if(product_updated_data[productId] != null){
				if(productsInfo[productId] == null) productsInfo[productId] = {};
...
var saveUpdatedCustomer = function(customer_updated_data){
	if(customer_updated_data == null || customer_updated_data == '') return;
	var customersInfo = {};
	if($D.jStorage.get('customersInfo') != null){
		...
		for(var customerId in customer_updated_data){
			if(customer_updated_data[customerId] != null){
				if(customersInfo[customerId] == null) customersInfo[customerId] = {};
...
var flushProductCache = function(){
	...
	if($D('#checkout')){
		$D('#checkout').click();
		if($('settings_area')) $('settings_area').removeClassName('left0px');
	}
...
var searchProductByCategory = function(categoryId){
	...
	if(productsInfo != ''){
		...
		for(var productId in productsInfo){
			...
			if(categories.indexOf(categoryId) >= 0 || categoryId == '0'){
				results.push(productId);
			}
		}
		if(results.length == 0){
...
var searchProductByKeyword = function(keyword){
	...
	if(productsInfo != ''){
		...
		for(var productId in productsInfo){
			var productData = productsInfo[productId];
			if(productData.searchstring.indexOf(keyword) >= 0){
				results.push(productId);
			}
		}
		if(results.length == 0){
...
var loadmoreProductByCategory = function(){
	...
	if(productsInfo != ''){
		...
		for(var productId in productsInfo){
			...
			if(categoryIds.indexOf(categoryId) >= 0 || categoryId == '0'){
				results.push(productId);
			}
		}
		if(results.length == 0){
			...
			if(results[maxKeyCustomer] != null){	
				if(customersInfo[results[maxKeyCustomer]] != null){
					...
					if(customerData.email != null){
						newresult += getCustomerHtml(results[maxKeyCustomer],customerData.email,customerData.firstname,customerData.lastname);
					}
				}
			}
			maxKeyCustomer++;
...
var saveBundleOption = function(productId,optionId,optionValue){
	...
	if($D.jStorage.get('bundle_options') != null){
		...
		if(bundle_options[productId] == null) bundle_options[productId] = {};
...
var saveBundleOptionQty = function(productId,optionId,optionValue){
	...
	if($D.jStorage.get('bundle_options_qty') != null){
		...
		if(bundle_options_qty[productId] == null) bundle_options_qty[productId] = {};
	...
	if(customerInCart != null){
		...
		if(addressType == 'billingAddress' && billingAddress != null){
			...
			billingInputs.each(function(input){
				...
				if(fieldkey == 'street1') input.value  = '';
				else if(fieldkey == 'street0') fieldkey = 'street';
				if(fieldkey != null && billingAddress[fieldkey] != null){
					input.value = billingAddress[fieldkey];
				}
				if(fieldkey == 'email') input.value = customerInCart.email;
			});
		}
...
var saveOrderToLocal = function(parameters){
	...
	if($D.jStorage.get('pending_orders') != null){
		...
		}else numberOrderSaved = 0;
		var newnumber = numberOrderSaved + 1;
		if(pending_orders['user_'+currentUserId] == null) pending_orders['user_'+currentUserId] = {};
...
var	deletePendingOrder = function(key){
	...
	if($D.jStorage.get('pending_orders') != null){
		pending_orders = JSON.parse($D.jStorage.get('pending_orders'));
		if(pending_orders['user_'+currentUserId][key] != null) delete  pending_orders['user_'+currentUserId][key];
		$D.jStorage.set('pending_orders',JSON.stringify(pending_orders));
	}
}
...
var saveConfigurableOptions = function(productId,optionCode,optionValue,optionLabel){
	...
	if($D.jStorage.get('configurable_options') != null){
		...
		if(configurable_options[productId] == null) configurable_options[productId] = {};
		if(configurable_options[productId][optionCode] == null) configurable_options[productId][optionCode] = {};
